...
if (!empty($_GET[""])) {
    $productId = isset($_GET["prod_id"]) ? htmlspecialchars($_GET['id']) : '';
    $quantity = isset($_POST['quantity']) ? htmlspecialchars($_POST['prod_qty']) : '';

    switch ($_GET["action"]) {
        case "add":
            if (!empty($quantity)) {
                $stmt = $db->prepare("SELECT * FROM kpjcafe_products where prod_id= ?");
